...
class Location {
	...
	// Location 생성자를 이용하여 위도 latitude 경도 longitude  값 설정해주기 (도시는 하지 않아도 됨.)
...
public class LocationManager {
	...
	public void read() {
		...
		for (int i = 0; i < 4; i++) {
			...
			StringTokenizer stl = new StringTokenizer(str, ","); //comma로 구분
			// city 값 받아오기 nextToken() 이용 (공백이 있을 시 trim() 사용)
			String city = stl.nextToken().trim();
			// lat, lng 값 받아오기 --> string (공백이 있을 시 trim() 사용) 이므로 Double.parseDouble() 함수 이용
	...
	public void printAll() {
		...
		Set<String> set = map.keySet(); //모든 키 받아옴
